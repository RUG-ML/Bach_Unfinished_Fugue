...
def pickle_objects(model_type: str, voice_num: int) -> None:
    """ A tool to save generated models, with varying window-sizes, into the ROM instead of overloading the RAM """
    for i in range(200):
        ...
        with open(os.path.join(os.path.dirname(os.path.abspath('.')), f"myTools\\pickles\\voice{voice_num+1}_{model_type}_w{i+1}.pickle"), "wb") as file_obj:
...
class Voice:
    ...
    def __init__(self, model_type, voice_num, win_size):
        self.model_type = model_type  # dummy property to make the objects explicitly distinguishable
...
def cv_loss(voice: object, cv_count: int):
...
def predict_note(voice: object, note: int, dataset_type: str, randomization=None, weight=2) -> [int, list]:
    ...
    likely_indices = get_n_likely_indices(normalized_preds, randomization)
...
def check_loss(voice: object, dataset_type: str, feedback=False) -> float:
    ...
    for weight in weights:
        ...
        for i in range(len(data_set_local)):
            y_idx = labels_local[i].argmax()  # index of the single "1" in the binary vector
            ...
            loss = np.log(yhat[1][y_idx])  # take log of the winning index
            losses.append(loss)
        losses_all_weights.append(-sum(losses)/len(losses))
    # return -sum(losses)/len(losses)
    # print(losses_all_weights)
    return losses_all_weights[0]


def generate_notes(voice_note: object, voice_octave: object, n: int) -> list:
    ...
    for i in range(n):
        ...
        while (yhat_note == -1 and yhat_octave != -2) or (yhat_octave == -2 and yhat_note != -1):
            yhat_octave = predict_note(voice_octave_copy, -1, dataset_type="all")[0]
...
def write_to_txt(pred_notes):
    pred_notes = pd.DataFrame([pred_notes[0], pred_notes[1],
                                        pred_notes[2], pred_notes[3]]).transpose()
    np.savetxt("pred410.txt", pred_notes, fmt="%s")


# 0) Read data "model\\F.txt"
my_path = 'C:\\Users\\varga\\Google Drive\\Hollandia\\Groningen\\School\\Courses\\1B\\' \
          'Machine Learning\\PROJECT\\CODE\\GITHUB\\Peter\\Bach_Unfinished_Fugue'
